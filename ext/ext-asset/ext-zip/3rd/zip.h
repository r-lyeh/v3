...
zip_t* zip_open(const char *file, const char *mode /*r,w,a*/);
zip_t* zip_open_handle(FILE*fp, const char *mode /*r,w,a*/);

    // only for (w)rite or (a)ppend mode
    bool zip_append_file(zip_t*, const char *entryname, const char *comment, FILE *in, unsigned compress_level);
    bool zip_append_file_timeinfo(zip_t*, const char *entryname, const char *comment, FILE *in, unsigned compress_level, struct tm *);
    bool zip_append_mem(zip_t*, const char *entryname, const char *comment, const void *in, unsigned inlen, unsigned compress_level);
    bool zip_append_mem_timeinfo(zip_t*, const char *entryname, const char *comment, const void *in, unsigned inlen, unsigned compress_level, struct tm *);

    // only for (r)ead mode
    int zip_find(zip_t*, const char *entryname); // convert entry to index. returns <0 if not found.
    unsigned zip_count(zip_t*);
        char*    zip_name(zip_t*, unsigned index);
        char*    zip_modt(zip_t*, unsigned index);
        unsigned zip_size(zip_t*, unsigned index);
        unsigned zip_hash(zip_t*, unsigned index);
        bool     zip_file(zip_t*, unsigned index); // is_file? (dir if name ends with '/'; file otherwise)
        bool     zip_test(zip_t*, unsigned index);
        char*    zip_comment(zip_t*, unsigned index);
        unsigned zip_codec(zip_t*, unsigned index);
        unsigned zip_offset(zip_t*, unsigned index);
        unsigned zip_excess(zip_t*, unsigned index);
        void*    zip_extract(zip_t*, unsigned index); // must free() after use
        bool     zip_extract_file(zip_t*, unsigned index, FILE *out);
        unsigned zip_extract_inplace(zip_t*, unsigned index, void *out, unsigned outlen_with_excess);
...
#ifndef ZIP_FREE
#define ZIP_FREE(p)      (void)!ZIP_REALLOC(p,0)
#endif

#ifndef ZIP_FPRINTF
#define ZIP_FPRINTF(...) ((void)0) // printf for logging
#endif

#ifndef ERR
#define ERR(NUM, ...)    (ZIP_FPRINTF(stderr, "" __VA_ARGS__), ZIP_FPRINTF(stderr, "(%s:%d) %s\n", __FILE__, __LINE__, strerror(errno)), /*fflush(stderr),*/ (NUM)) // (NUM)
#endif
...
static char* ZIP_STRDUP(const char *s) { int bl = strlen(s)+1; return memcpy(ZIP_REALLOC(0,bl), s, bl); }
...
int jzReadEndRecord(FILE *fp, JZEndRecord *endRecord) {
    ...
    ZIP_FPRINTF(stdout, "end)\n\tsignature: 0x%X\n", e->signature ); // 0x06054b50
    ZIP_FPRINTF(stdout, "\tdiskNumber: %d\n", e->diskNumber ); // unsupported
    ZIP_FPRINTF(stdout, "\tcentralDirectoryDiskNumber: %d\n", e->centralDirectoryDiskNumber ); // unsupported
    ZIP_FPRINTF(stdout, "\tnumEntriesThisDisk: %d\n", e->numEntriesThisDisk ); // unsupported
    ZIP_FPRINTF(stdout, "\tnumEntries: %d\n", e->numEntries );
    ZIP_FPRINTF(stdout, "\tcentralDirectorySize: %u %#x\n", e->centralDirectorySize, e->centralDirectorySize );
    ZIP_FPRINTF(stdout, "\tcentralDirectoryOffset: %u %#x\n", e->centralDirectoryOffset, e->centralDirectoryOffset );
    ZIP_FPRINTF(stdout, "\tzipCommentLength: %d\n---\n", e->zipCommentLength );
...
int jzReadCentralDirectory(FILE *fp, JZEndRecord *endRecord, JZRecordCallback callback, void *user_data, void *user_data2) {
    ...
    for(int i=0; i<endRecord->numEntries; i++) {
        ...
        ZIP_FPRINTF(stdout, "\tsignature: %u %#x\n", g->signature, g->signature); // 0x02014B50
        ZIP_FPRINTF(stdout, "\tversionMadeBy: %u %#x\n", g->versionMadeBy, g->versionMadeBy); // unsupported
        ZIP_FPRINTF(stdout, "\tversionNeededToExtract: %u %#x\n", g->versionNeededToExtract, g->versionNeededToExtract); // unsupported
        ZIP_FPRINTF(stdout, "\tgeneralPurposeBitFlag: %u %#x\n", g->generalPurposeBitFlag, g->generalPurposeBitFlag); // unsupported
        ZIP_FPRINTF(stdout, "\tcompressionMethod: %u %#x\n", g->compressionMethod, g->compressionMethod); // 0-store,8-deflate
        ZIP_FPRINTF(stdout, "\tlastModFileTime: %u %#x\n", g->lastModFileTime, g->lastModFileTime);
        ZIP_FPRINTF(stdout, "\tlastModFileDate: %u %#x\n", g->lastModFileDate, g->lastModFileDate);
        ZIP_FPRINTF(stdout, "\tcrc32: %#x\n", g->crc32);
        ZIP_FPRINTF(stdout, "\tcompressedSize: %u\n", g->compressedSize);
        ZIP_FPRINTF(stdout, "\tuncompressedSize: %u\n", g->uncompressedSize);
        ZIP_FPRINTF(stdout, "\tfileNameLength: %u\n", g->fileNameLength);
        ZIP_FPRINTF(stdout, "\textraFieldLength: %u\n", g->extraFieldLength); // unsupported
        ZIP_FPRINTF(stdout, "\tfileCommentLength: %u\n", g->fileCommentLength); // unsupported
        ZIP_FPRINTF(stdout, "\tdiskNumberStart: %u\n", g->diskNumberStart); // unsupported
        ZIP_FPRINTF(stdout, "\tinternalFileAttributes: %#x\n", g->internalFileAttributes); // unsupported
        ZIP_FPRINTF(stdout, "\texternalFileAttributes: %#x\n", g->externalFileAttributes); // unsupported
        ZIP_FPRINTF(stdout, "\trelativeOffsetOflocalHeader: %u %#x\n", g->relativeOffsetOflocalHeader, g->relativeOffsetOflocalHeader);
...
int jzReadData(FILE *fp, JZGlobalFileHeader *header, void *out) {
    if(header->compressionMethod == 0) { // Store - just read it
        ...
        void *in = ZIP_REALLOC(0, inlen + 8); // small excess as some decompressors are really wild with output buffers (lz4x)
...
int zip__callback(FILE *fp, int idx, JZGlobalFileHeader *header, char *filename, void *extra, char *comment, void *user_data) {
    ...
    z->entries = ZIP_REALLOC(z->entries, (++z->count) * sizeof(struct zip_entry) );

    struct zip_entry *e = &z->entries[index];
    e->header = *header;
    e->filename = ZIP_STRDUP(filename);
    e->offset = ftell(fp);
    e->extra = ZIP_REALLOC(0, header->extraFieldLength);
    memcpy(e->extra, extra, header->extraFieldLength);
    e->comment = ZIP_STRDUP(comment);
...
int zip_find(zip_t *z, const char *entryname) {
    int zip_debug = ZIP_DEBUG; ZIP_DEBUG = 0;
    if(zip_debug) ZIP_FPRINTF(stdout, "zip_find(%s)\n", entryname);
    if( z->in ) for( int i = z->count; --i >= 0; ) { // in case of several copies, grab most recent file (last coincidence)
        if(zip_debug) ZIP_FPRINTF(stdout, "\t%d) %s\n", i, z->entries[i].filename);
        if( 0 == strcmp(entryname, z->entries[i].filename)) return i;
    }
    return -1;
}

bool zip_file(zip_t *z, unsigned index) { // is_file? (dir if attrib&15 or name ends with '/'; file otherwise)
...
unsigned zip_excess(zip_t *z, unsigned index) {
    if( z->in && index < z->count ) {
...
unsigned zip_extract_inplace(zip_t *z, unsigned index, void *out, unsigned outlen) {
    if( z->in && index < z->count ) {
...
void *zip_extract(zip_t *z, unsigned index) { // must free()
    if( z->in && index < z->count ) {
        unsigned outlen = (unsigned)z->entries[index].header.uncompressedSize;
        char *out = (char*)ZIP_REALLOC(0, outlen + 1 + zip_excess(z, index));
        unsigned ret = zip_extract_inplace(z, index, out, outlen);
        return ret ? (out[outlen] = '\0', out) : (out = ZIP_REALLOC(out, 0));
    }
    return NULL;
}

bool zip_extract_file(zip_t *z, unsigned index, FILE *out) {
...
bool zip_test(zip_t *z, unsigned index) {
...
bool zip_append_file(zip_t *z, const char *entryname, const char *comment, FILE *in, unsigned compress_level) {
...
bool zip_append_file_timeinfo(zip_t *z, const char *entryname, const char *comment, FILE *in, unsigned compress_level, struct tm* timeinfo) {
    ...
    *e = zero;
    e->filename = ZIP_STRDUP(entryname);
    e->comment = comment ? ZIP_STRDUP(comment) : 0;
    ...
    // if(comp) comp = ZIP_REALLOC(comp, 1); // re-entry optimization: hopefully the allocator will optimize this out (N>1-byte)
    // if(data) data = ZIP_REALLOC(data, 1); // re-entry optimization: hopefully the allocator will optimize this out (N>1-byte)
    ...
    comp = ZIP_REALLOC(comp, compSize);
    if(comp == NULL) goto cant_compress;

    data = ZIP_REALLOC(data, dataSize + 8); // small excess as some compressors are really wild when reading from buffers (lz4x)
    ...
    if(bytes != dataSize) {
        return ERR(false, "Failed to read file in full (%u vs. %u bytes)", (unsigned)bytes, (unsigned)dataSize);
    }
...
//  comp = ZIP_REALLOC(comp, 0); // see re-entry optimization above
//  data = ZIP_REALLOC(data, 0); // see re-entry optimization above
    return true;
}

bool zip_append_mem(zip_t *z, const char *entryname, const char *comment, const void *in, unsigned inlen, unsigned compress_level) {
    if( !entryname ) return ERR(false, "No filename provided");
...
bool zip_append_mem_timeinfo(zip_t *z, const char *entryname, const char *comment, const void *in, unsigned inlen, unsigned compress_level, struct tm* timeinfo) {
    if( !in ) return ERR(false, "No input file provided");
    if( !entryname ) return ERR(false, "No filename provided");
    if( !timeinfo ) return ERR(false, "No timeinfo provided");
...
zip_t* zip_open_handle(FILE *fp, const char *mode) {
    if( !fp ) return ERR(NULL, "cannot open file for %s mode", mode);
    zip_t zero = {0}, *z = (zip_t*)ZIP_REALLOC(0, sizeof(zip_t));
...
zip_t* zip_open(const char *file, const char *mode /*r,w,a*/) {
    ...
    zip_t *z = zip_open_handle(fp, mode);
...
void zip_close(zip_t* z) {
    ...
    // clean up
    for(unsigned i = 0; i < z->count; ++i ) {
        ZIP_FREE(z->entries[i].filename);
        if(z->entries[i].extra)   ZIP_FREE(z->entries[i].extra);
        if(z->entries[i].comment) ZIP_FREE(z->entries[i].comment);
    }
    if(z->entries) ZIP_FREE(z->entries);
    zip_t zero = {0}; *z = zero; ZIP_FREE(z);
}
