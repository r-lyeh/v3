...
#define state   spine_demo_state
#define frame   spine_demo_frame
#define cleanup spine_demo_cleanup
...
static void image_data_loaded(const void *ptr, int size, void *userdata);
...
static void init(void) {
    ...
    {
        file_t f = file("raptor-pma.atlas");
        state.load_status.atlas = (load_status_t){
            .loaded = true,
            .data = (sspine_range){ f.bin, f.len },
        };
    }
    {
        file_t f = file("raptor-pro.skel");
...
static void create_spine_objects(void) {
    ...
    for (int img_index = 0; img_index < num_images; img_index++) {
        ...
        char path_buf[512];
        file_t f = file(img_info.filename.cstr);
