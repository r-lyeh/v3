...
int fps_wait0(void) {
    ONCE if( target_fps < 0 ) {
        ...
        float monitor_fps = app_framerate();
        float asked = flagi("--fps", (int)(monitor_fps+0.5)+2); /* 0 for immediate updates, 1 for updates synchronized with the vertical retrace, -1 for adaptive vsync, N to any given fps number. defaults to monitor Hz */
