...
unsigned dd_state_begin(mat44 proj, mat44 view) { // inits vao+vbo, flushes geometry, resets color
    ONCE {
    const char *vss[] = { preamble, dd_vs }; int vert = shader("vs", 2, vss);
    const char *fss[] = { preamble, dd_fs_line_solid }; int frag = shader("fs", 2, fss);
    ...
    const char *fss2[] = { preamble, dd_fs_point }; int frag2 = shader("fs", 2, fss2);
