...
#include "art/pico_math.h"

#include "art/shaders.h"

#pragma push_macro("app")
#undef  app
#define app appSCN
#define node_t node_tSCN
#define vertex_t vertex_tSCN
#define load_texture load_textureSCN
...
{
    image_t img = image(.url = file, .c = 4, .flip = 1);

    assert(img.bitmap && img.c == 4);

    size_t size = img.w * img.h * img.c;
    pg_texture_t* tex = pg_create_texture(app.ctx, img.w, img.h, img.bitmap, size, NULL);
...
void app_startup(void)
{
    ...
    app_create(.title = "Scene Graph Example", "0.50");

    app.screen_w = app_width();
    app.screen_h = app_height();
}
...
scenegraph_t* sg_build(int scene_w, int scene_h)
{
    ...
    // Load texture
    pg_texture_t* bg_tex = load_texture("space.webp");
    ...
    pg_texture_t* star_tex = load_texture("star.webp");
    ...
    pg_texture_t* ship_tex = load_texture("ship.webp");
    ...
    pg_texture_t* jet_tex = load_texture("jet.webp");
...
int main()
{
                                               ...
                                               &(pg_pipeline_opts_t)
    {
        .layout =
        {
            .attrs =
            {
                [ATTR_sprite_a_pos]   = { .format = PG_VERTEX_FORMAT_FLOAT3,
                                      .offset = offsetof(vertex_t, pos) },

                [ATTR_sprite_a_color] = { .format = PG_VERTEX_FORMAT_FLOAT4,
                                      .offset = offsetof(vertex_t, color) },

                [ATTR_sprite_a_uv]    = { .format = PG_VERTEX_FORMAT_FLOAT2,
...
#pragma pop_macro("app")
#undef node_t // node_tSCN
#undef vertex_t // vertex_tSCN
#undef load_texture // load_textureSCN
#undef MAX_CHILDREN // 5
#undef FIXED_STEP // (1.0 / 50.0)
