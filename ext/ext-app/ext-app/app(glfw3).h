...
//  if( opt.scale >= 1 ) xpos = ypos = 0, ww = mw, wh = mh;
    ...
    glfwWindowHint(GLFW_DECORATED, opt.scale < 1);
    ...
    // center windowed
    glfwWindowHint(GLFW_POSITION_X, xpos + (ww - opt.w) / 2);
    glfwWindowHint(GLFW_POSITION_Y, ypos + (wh - opt.h) / 2);
    ...
    event(on_graphics_context_create);
    ...
    if( opt.visible ) glfwShowWindow(window);
            ...
            lputs(va("GPU compat: %d, depth: %d, stencil: %d, aa: %d, srgb: %d", profile, depth_bits, stencil_bits, samples, srgb_enabled));
