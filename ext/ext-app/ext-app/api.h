...
#define on_graphics_context_create on_graphics_context_create
void on_graphics_context_create(void *fn, void *arg);
...
#define app_create(...) app_create(CAST(app_create_flags_t) {.resizable = 1, .visible = 1, .debug = !V3_RETAIL, .silent = V3_RETAIL, ##__VA_ARGS__} )
...
void on_graphics_context_create(void *fn, void *arg) { hook(on_graphics_context_create, fn, arg); }
