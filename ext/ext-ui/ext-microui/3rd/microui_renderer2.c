...
static void push_quad2(mu_Rect dst, mu_Rect src, mu_Color color) {
  if (buf_idx == BUFFER_SIZE) { r_flush2(); }
...
void r_draw_rect2(mu_Rect rect, mu_Color color) {
  push_quad2(rect, atlas[ATLAS_WHITE], color);
}


void r_draw_text2(const char *text, mu_Vec2 pos, mu_Color color) {
  ...
  for (const char *p = text; *p; p++) {
    ...
    dst.w = src.w;
    dst.h = src.h;
    push_quad2(dst, src, color);
    dst.x += dst.w;
  }
}


void r_draw_icon2(int id, mu_Rect rect, mu_Color color) {
  mu_Rect src = atlas[id];
  int x = rect.x + (rect.w - src.w) / 2;
  int y = rect.y + (rect.h - src.h) / 2;
  push_quad2(mu_rect(x, y, src.w, src.h), src, color);
}


int r_get_text_width2(const char *text, int len) {
  int res = 0;
...
int r_get_text_height2(void) {
  return 18;
}


void r_set_clip_rect2(mu_Rect rect) {
  r_flush2();
  glScissor(rect.x, app_height() - (rect.y + rect.h), rect.w, rect.h);
}


void r_clear2(mu_Color clr) {
  r_flush2();
  glClearColor(clr.r / 255.0f, clr.g / 255.0f, clr.b / 255.0f, clr.a / 255.0f);
  glClear(GL_COLOR_BUFFER_BIT);
}

void r_present2(void) {
  r_flush2();
}

void r_draw_image2(mu_Rect dst, mu_Rect src, void *texture_handle) {
