...
// Basically, the editor app is a compositor of multiple docked windows.
...
//
// editor = for each window[] : display(.texture = window.draw(user.inputs))

#include "editor_app.h"

// We can say each window is a standalone app, which renders things into its own viewport.
//
...
// Because of human (debugging) reasons, the messaging happens instantly and is not deferred or queued at any time.
...
// Similarly to the case above, the window matrix is the place where all the existing windows are managed:
...
// Ideally, at the end of the day, each window should be able to:
// - Spawn things
// - Select things
// - Transform things
// - Align things
...
// - Import foreign things (read popular asset type > save lossless native type)
// - Export foreign things (read lossless > save external portable format)
// - Edit things (properties)
...
// - Drag n drop items to other windows: each drag n drop item is a url to a disk file.
