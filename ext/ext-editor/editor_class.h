...
typedef struct class_t {
    #define EXPAND_METHOD(x) var_t (*x)();
    EDITOR_FOREACH_VERB(EXPAND_METHOD)
    #undef EXPAND_METHOD
} class_t;
#pragma pack(pop)
...
void    editor_class_dump(FILE *out, const char *mask);
...
static int editor_push_row(void *properties_table) {
    ...
    static int once = 0; if(!once) { once = 1;
        char* property_names[] = {
            ...
            EDITOR_FOREACH_VERB(DEFINE_LITERAL)
