...
AUTORUN {
    editor_window my_window = editor_window(.title = "Game view");
    editor_add_window(my_window);
}

int main() {

    editor_class_dump(stdout, "*");

    unsigned frame = 0, speed = 0;
    camera_t cam = { .pos=vec3(12,16,30), .yaw=-116, .pitch=-25 };

    for( app_create(.title = "v3(editor)", .scale = 100, .transparent=1); app_swap() && !key('\x1b'); frame += speed ) {
        ...
        editor_draw_windows(color_texture);
