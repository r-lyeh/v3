...
#define vgl_blend(on) \
    (on ? (glEnable(GL_BLEND),glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)) : glDisable(GL_BLEND))
#define vgl_depth(read, write) \
    (read  ? (glEnable(GL_DEPTH_TEST), glDepthFunc(read)) : glDisable(GL_DEPTH_TEST)) \
    (write ? glDepthMask(GL_TRUE) : glDepthMask(GL_FALSE))

// enum { off = 0, on = 1 }; enum { LESS = GL_LESS };
// vgl_blend(off); // off, on, ADD, MUL
// vgl_depth_read(LESS); // [off,on,LESS,GEQUAL]
// vgl_depth_write(on);
// vgl_cull(0); // 0(off),1(front),2(two-sided),2(back)
// vgl_ccw(off);
// vgl_stencil(todo);
