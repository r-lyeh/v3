...
int main() {
    ...
    for( app_create(.title = "hello v3", .scale=100, .transparent=1); app_swap(.color=0) && !key('\x1b'); ) {
        if( ui_window("hello v3", 0) ) {
            for( int i = 1; i <= demos_num; ++i ) {
                ...
                if( choice == 1 ) app_show(0), system(va(ifndef(V3_WINDOWS, "./") "%s %d", argv0(), i)), app_show(1);
