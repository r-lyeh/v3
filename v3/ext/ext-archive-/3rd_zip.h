...
void *zip_extract(zip_t *z, unsigned index) { // must free()
    if( z->in && index < z->count ) {
        ...
        return ret ? (out[outlen] = '\0', out) : (out = REALLOC(out, 0));
    }
    return NULL;
}
...
//  comp = REALLOC(comp, 0); // see re-entry optimization above
//  data = REALLOC(data, 0); // see re-entry optimization above
    return true;
}
...
zip_t* zip_open_handle(FILE *fp, const char *mode) {
    ...
    zip_t zero = {0}, *z = (zip_t*)REALLOC(0, sizeof(zip_t));
...
void zip_close(zip_t* z) {
    ...
    zip_t zero = {0}; *z = zero; (void)!REALLOC(z, 0);
}
