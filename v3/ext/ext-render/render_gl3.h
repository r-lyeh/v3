void*     app_create_gl3(const char *appname, const char *flags); // @todo: flags += " gl33 debug"
...
void      draw_uint_tris(unsigned vao, unsigned tris, unsigned program);

#if CODE && !defined GL2
#pragma once

void* app_create_gl3(const char *appname, const char *flags) {
    RGFW_init();
    ...
    void *handle = app_create(appname, flags);
    // gladLoader
    return handle;
}
...
unsigned buffer(int slot, const char *fmt, const void *bin, unsigned len) {
    ...
    //
    // examples:
    // `2f2f4b`, `!!3f2f2i`, `!vertex2f texcoord3f color4b`, `2f2F`
