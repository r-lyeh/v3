...
#ifdef GL2
#include "3rd/microui_renderer2.c"
#else
#include "3rd/microui_renderer3.c"
#endif
...
int ui_buttons_(int num, ...) {
    if( num == 2 ) mu_layout_row(ui_ctx, 2, (int[]) { -50, -1 }, 0);
    else
    if( num ) die("ui_buttons: unsupported num of buttons");
...
int ui_slider_(const char *txt, float *f, float minf, float maxf) {
    //mu_layout_row(ui_ctx, 2, (int[]) { 50, -1 }, 0);
    //mu_layout_row(ui_ctx, 2, (int[]) { -200, -1 }, 0);
    return ui_label_(txt), mu_slider(ui_ctx, f, minf, maxf);
}
int ui_float_(const char *txt, float *f) {
    return ui_slider_(txt, f, -FLT_MAX, FLT_MAX);
}
...
void ui_begin_(void *arg) { // must be initialized after window is created
    ...
    /* handle RGFW events */
    extern RGFW_window* app;
    extern int mouse_x, mouse_y, mouse_b[], mouse_wheel;
    extern int keyboard[], text[], text_count, up[], up_count, down[], down_count;

    mu_input_mousemove(ui_ctx, mouse_x, mouse_y);
    mu_input_scroll(ui_ctx, 0, (mouse_wheel / 100.) * -30);

    for( int i = 0; i < up_count; ++i ) {
        int c = key_map[up[i] & 0xff];
        if( c < 3 ) mu_input_mouseup(ui_ctx, mouse_x, mouse_y, button_map[c]);
        else        mu_input_keyup(ui_ctx, c);
    }
    for( int i = 0; i < down_count; ++i ) {
        int c = key_map[down[i] & 0xff];
        if( c < 3 ) mu_input_mousedown(ui_ctx, mouse_x, mouse_y, button_map[c]);
        else        mu_input_keydown(ui_ctx, c);
    }
...
AUTORUN {
    api_hook(ui_begin_, ui_end_, ui_ctx);
}
