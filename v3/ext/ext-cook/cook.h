...
const char *cook__( const char *source) {
    ...
    for each_string(token, recipe->script, " \n") { // @todo: multiline support: \n>& \n>;
        stradd(&batch, " ");
        /**/ if( strstri(token, ".EXE") ) stradd(&batch, path_norm( va("v3/%.*s%s", (int)(strlen(token)-4), token, ifdef(win32,".exe", ifdef(linux,".linux",".osx")) )));
        else if( strstri(token, "ext/") || strstri(token, "ext\\") ) stradd(&batch, path_norm( va("v3/%s", token) ));
        else if( !strcmp(token, "REN") ) stradd(&batch, ifdef(win32, "move /y", "mv -f"));
        else if( !strcmp(token, "DEL") ) stradd(&batch, ifdef(win32, "del", "rm"));
        else if( !strcmp(token, "NUL") ) stradd(&batch, ifdef(win32, "nul", "/dev/null"));
...
const char *cook_( const char *source ) {
    ...
    char target[DIR_MAX];
    snprintf(target, DIR_MAX, "%s.%s", path(source), path_name(source));
