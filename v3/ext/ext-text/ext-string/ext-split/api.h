...
// syntax sugar. usage: for each_string(s, "hello world", " ") puts(s);
#define each_string(var,str,seps) (char **lst = strsplit(str, seps, 0), *var; !!(var = lst[0]); ++lst)
...
char** strsplit_(const char *text, const char *separators, int *count) { // do not free()
    ...
    static char **lists[16] = {0}, *bufs[16]= {0};
    static int slot = 0; slot = (slot + 1) % COUNTOF(lists);
