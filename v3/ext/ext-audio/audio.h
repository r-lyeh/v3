...
#include "../ext-thread/minithread.h"
...
audio_t audio_(audio_t f) {
    if( !f.handle && f.url ) {
        ifndef(retail, strncpy(f.debug, f.url, COUNTOF(f.debug)));
        file_t x = file(f.url);
...
static
void speaker_drop_(void *p) {
    speaker_t *s = (speaker_t*)p;
    speaker_stop_(s);
...
void audio_init(void) {
    if(!saudio_isvalid()) {
        ...
        if( saudio_isvalid() )
            puts(va("audio: %dKHz, %d channels, %d frames", saudio_sample_rate()/1000, saudio_channels(), saudio_buffer_frames()));
        else
            die(va("Cannot initialize audio: %dHz rate, %d channels, %d frames", saudio_sample_rate(), saudio_channels(), saudio_buffer_frames()));
    }
}
