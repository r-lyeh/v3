...
#elif DEFINED(gcc) && !DEFINED(ems) && !DEFINED(mingw) // maybe DEFINED(linux) is enough?
#include <execinfo.h>  // backtrace, backtrace_symbols
#elif DEFINED(win32) // && !defined __TINYC__
...
#if DEFINED(tcc) && DEFINED(linux)
    return "";
#endif
#if DEFINED(ems) // there is a stack overflow failure somewhere in the impl below
