...
if [ "$1" = "tidy" ]; then
    ...
    rm .*.zip 2> /dev/null
    rm *.linux 2> /dev/null
    rm v3unity.c 2> /dev/null
    rm -rf *.dSYM 2> /dev/null
    exit
fi

if [ "$1" = "unity" ]; then
    cc v3.c -E -I. > v3unity.c
    exit
fi
...
echo cc v3.c -o v3 -lm -ldl -lasound -lpthread -g $*
time cc v3.c -o v3 -lm -ldl -lasound -lpthread -g $* || exit
...
echo cc -ObjC v3.c -o v3 -framework cocoa -framework iokit -framework audiotoolbox -g $*
time cc -ObjC v3.c -o v3 -framework cocoa -framework iokit -framework audiotoolbox -g $* || exit
...
if "%1"=="tidy" (
    del v3.rc v3.res v3unity.c .*.zip *.o *.obj *.pdb *.ilk *.exp *.lib *.exe *.res >nul 2>nul
...
if "%1"=="unity" (
    echo !CC! v3.c -E ^> v3unity.c
         !CC! v3.c -E  > v3unity.c || goto error
    goto ok
)
